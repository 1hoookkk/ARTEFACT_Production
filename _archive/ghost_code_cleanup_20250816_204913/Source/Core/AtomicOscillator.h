...
// RAII class for denormal suppression
...
public:
    ...
    {
        // Suppress denormals for this processing block
        AtomicOscScopedNoDenormals noDenormals;
    ...
    AtomicOscillator& getOscillator(size_t index) noexcept
    {
        ...
        if (index >= BANK_SIZE) [[unlikely]]
        {
            ...
            {
                // RT-SAFE: Removed logging from audio thread
            }
...
public:
    ...
    Statistics getStatistics() const noexcept
    {
        ...
        stats.usedBanks = (usedBanks > 0) ? usedBanks : 0;
