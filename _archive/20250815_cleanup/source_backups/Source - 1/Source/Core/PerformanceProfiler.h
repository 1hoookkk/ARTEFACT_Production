...
private:
    ...
    mutable juce::CriticalSection timingDataLock;
    std::unordered_map<std::string, std::unique_ptr<TimingData>> timingData;
    
    // Pipeline-specific data
    std::array<std::unique_ptr<TimingData>, 7> pipelineData; // One for each PipelineStage
    ...
    mutable juce::CriticalSection activeTimersLock;
    std::unordered_map<std::string, ActiveTimer> activeTimers;
    ...
    mutable juce::CriticalSection alertsLock;
