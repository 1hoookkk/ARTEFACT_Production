    ...
    : processor(processorToUse)
{
    ...
    // DEFERRED TIMER START: Prevent RT violation during construction
    juce::MessageManager::callAsync([safePtr = juce::Component::SafePointer<HeaderBarComponent>(this)]()
    {
        if (safePtr)
            safePtr->startTimerHz(10); // 10Hz for LED blinking and beat indicator
    });
}
