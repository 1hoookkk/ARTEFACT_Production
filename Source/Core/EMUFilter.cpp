...
{
    // Add subtle noise for vintage character
    float noise = 0.0f;
    if (SpectralCanvas::Determinism::IsEnabled())
    {
        // Deterministic noise using a global seeded LCG
        static SpectralCanvas::Determinism::Lcg32 lcg(SpectralCanvas::Determinism::GetSeed() ^ 0x9E3779B9u);
        noise = lcg.nextFloat01() * 0.0001f * filterAge;
    }
    else
    {
        noise = vintageRandom.nextFloat() * 0.0001f * filterAge;
    }
